	...
	public function InsertImagesProduct($producto_id,$params)
	{
		...
		for($i=0;$i<$images;$i++)
		{
			if($params['name'][$i])
			{
				...
				$sql="INSERT INTO imagenes_productos VALUES(0,:producto,:name,:route)";
	...
	public function GetProductsSell()
	{
		$sql="SELECT p.producto_id,p.producto_sku,p.producto_name,
				...
				FROM productos p
                                ...
                                LEFT JOIN materiales m USING (material_id)
				WHERE producto_type IN('U','V')
				ORDER BY p.producto_name ASC";
		...
		$sql="SELECT p.producto_id,p.producto_sku,p.producto_name,proveedor_id,
				...
				IF((SELECT imagen_route
					...
					(SELECT imagen_route
						...
						WHERE ip.producto_id=p.producto_id
						ORDER BY ip.imagen_id ASC
						LIMIT 0,1),'".FINAL_URL."img/imagen-no.png') AS imagen_principal
				FROM productos p
